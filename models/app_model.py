...
class AppModel:
    ...
    def get_client(self, client_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        return next((c.copy() for c in self.clients if c.get('id') == client_id), None)
    ...
    def save_templates(self) -> bool:
        ...
        try:
            # Créer un fichier temporaire d'abord
            temp_file = f"{template_file}.tmp"
            with open(temp_file, 'w', encoding='utf-8') as f:
                json.dump(self.templates, f, indent=2, ensure_ascii=False)
            
            # Remplacer l'ancien fichier par le nouveau
            if os.path.exists(template_file):
                os.replace(temp_file, template_file)
            else:
                os.rename(temp_file, template_file)
    ...
    def get_template(self, template_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        return next((t.copy() for t in self.templates if t.get('id') == template_id), None)
    ...
    def get_document(self, document_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        return next((d.copy() for d in self.documents if d.get('id') == document_id), None)
    ...
    def search_documents(self, query: str) -> List[Dict[str, Any]]:
        """
        Recherche des documents par titre, type ou description
        
        Args:
            query: Terme de recherche
        
        Returns:
            list: Liste des documents correspondant à la recherche
        """
        query = query.lower()
        results = []
        
        for document in self.documents:
            if (query in document.get('title', '').lower() or
                query in document.get('type', '').lower() or
                query in document.get('description', '').lower()):
                results.append(document.copy())
        
        return results
    
    def get_documents_by_client(self, client_id: str) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            list: Liste des documents du client
        """
        return [d.copy() for d in self.documents if d.get('client_id') == client_id]
    
    def get_documents_by_template(self, template_id: str) -> List[Dict[str, Any]]:
        """
        Récupère tous les documents créés à partir d'un modèle
        
        Args:
            template_id: ID du modèle
        
        Returns:
            list: Liste des documents basés sur le modèle
        """
        return [d.copy() for d in self.documents if d.get('template_id') == template_id]
    
    def filter_documents(self, client_id: str = None, template_id: str = None, 
                      document_type: str = None, date_from: str = None, 
                      date_to: str = None, query: str = None) -> List[Dict[str, Any]]:
        """
        Filtre les documents selon plusieurs critères
        
        Args:
            client_id: ID du client (optionnel)
            template_id: ID du modèle (optionnel)
            ...
            query: Terme de recherche dans le titre ou la description (optionnel)
        
        Returns:
            list: Liste des documents correspondant aux critères
        """
        ...
        # Filtrer par modèle
        if template_id:
            filtered_docs = [d for d in filtered_docs if d.get('template_id') == template_id]
        
        # Filtrer par type
        if document_type:
            filtered_docs = [d for d in filtered_docs if d.get('type') == document_type]
        
        # Filtrer par date
        if date_from:
            filtered_docs = [d for d in filtered_docs if d.get('date', '') >= date_from]
        
        if date_to:
            filtered_docs = [d for d in filtered_docs if d.get('date', '') <= date_to]
        
        # Filtrer par recherche
        if query:
            query = query.lower()
            filtered_docs = [d for d in filtered_docs if 
                            query in d.get('title', '').lower() or 
                            query in d.get('description', '').lower()]
        
        return [d.copy() for d in filtered_docs]
