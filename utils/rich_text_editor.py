...
class RichTextEditor(ctk.CTkFrame):
    ...
    def set_content(self, content):
        """Définit le contenu de l'éditeur de texte"""
    ...
    def get_html_content(self):
        """
        Retourne le contenu de l'éditeur formaté en HTML
        ...
        try:
            ...
            # Position actuelle du curseur et de la fin du texte
            start = "1.0"
            end = self.text_area.index(tk.END)
            
            # Parcourir le texte par paragraphes
            current = start
            while self.text_area.compare(current, "<", end):
                # Trouver la fin du paragraphe actuel
                next_para = self.text_area.search("\n", current, tk.END)
                if not next_para:
                    next_para = end
                
                # Récupérer le texte du paragraphe
                paragraph_text = self.text_area.get(current, next_para)
                
                # Déterminer l'alignement du paragraphe
                alignment = "left"
                for tag in self.text_area.tag_names(current):
                    if tag in ["left", "center", "right"]:
                        alignment = tag
                
                # Ouvrir le paragraphe avec l'alignement approprié
                html.append(f'<p style="text-align: {alignment};">')
                
                # Traiter le texte caractère par caractère pour les styles
                pos = current
                while self.text_area.compare(pos, "<", next_para):
                    # Récupérer les tags à cette position
                    tags = self.text_area.tag_names(pos)
                    
                    # Déterminer jusqu'où ces tags s'appliquent
                    next_change = self.text_area.index(f"{pos}+1c")
                    for tag in tags:
                        tag_range = self.text_area.tag_nextrange(tag, pos, next_para)
                        if tag_range:
                            if self.text_area.compare(tag_range[1], "<", next_change):
                                next_change = tag_range[1]
                    
                    # Extraire le texte avec ces tags
                    styled_text = self.text_area.get(pos, next_change)
                    
                    # Appliquer les styles (ouvrir les balises)
                    styled_html = styled_text
                    ...
                    for tag in tags:
                        if tag == "bold":
                            styled_html = f"<strong>{styled_html}</strong>"
                        elif tag == "italic":
                            styled_html = f"<em>{styled_html}</em>"
                        elif tag == "underline":
                            styled_html = f'<span style="text-decoration: underline;">{styled_html}</span>'
                        elif tag == "strikethrough":
                            styled_html = f'<span style="text-decoration: line-through;">{styled_html}</span>'
                        elif tag == "variable":
                            styled_html = f'<span style="color: blue;">{styled_html}</span>'
                        elif tag.startswith("color-"):
                            color = "#" + tag[6:]
                            color_style = f"color: {color};"
                        elif tag.startswith("bg-color-"):
                            bg_color = "#" + tag[9:]
                            bg_color_style = f"background-color: {bg_color};"
                    
                    # Appliquer les styles composés
                    if color_style or bg_color_style:
                        styled_html = f'<span style="{color_style}{bg_color_style}">{styled_html}</span>'
                    
                    # Ajouter au HTML
                    html.append(styled_html)
                ...
                current = self.text_area.index(f"{next_para}+1c")
            ...
            return f"<html><body><p>Erreur lors de la conversion en HTML: {e}</p></body></html>"
