...
class DocumentView:
    ...
    def create_folder_card(self, folder_id, folder_name, count, row, col):
        ...
        # IMPORTANT: Utiliser height et width fixes + grid_propagate(False) est crucial
        ...
        # Unité (sous-dossiers ou documents)
        unit = "sous-dossiers" if folder_id in ["date", "type", "client"] else "documents"
        
        # Nombre de documents ou sous-dossiers
        ...
        # Configurer les gestionnaires d'événements - NOUVELLE APPROCHE SANS LAMBDA
    ...
    def _show_type_folders(self):
        """Affiche les dossiers organisés par type de document - version corrigée"""
        # Nettoyer la vue
        self.documents_grid.pack_forget()
        self.no_documents_label.pack_forget()
        ...
        # Analyser les documents par type - CORRECTION CRITIQUE ICI
        types = {}
        for doc in self.model.documents:
            # Extraire le type
            doc_type = doc.get("type", "").lower()
        ...
        for doc_type, docs in sorted(types.items()):
            # Appeler la méthode corrigée pour créer la carte
    ...
    def _show_client_folders(self):
        """Affiche les dossiers organisés par client - version corrigée et améliorée"""
        # Nettoyer la vue
        self.documents_grid.pack_forget()
        self.no_documents_label.pack_forget()
    ...
    def _create_client_folder_card(self, client_id, client_name, count, row, col):
        """Crée une card pour un dossier de client - version corrigée"""
        # Cadre principal avec taille fixe
        card = ctk.CTkFrame(self.folders_grid)
        card.grid(row=row, column=col, padx=10, pady=10, sticky="nsew")
        card.configure(width=200, height=200)
    ...
    def show_documents_by_client(self, client_id, client_name):
        ...
        if not client_exists:
            logger.warning(f"Client {client_id} non trouvé, tentative de récupération depuis le cache")
            # Essayer de récupérer le nom depuis le cache
            cached_name = self._get_client_name_cached(client_id)
            if cached_name != "Client inconnu":
                client_name = cached_name
            else:
                logger.error(f"Client {client_id} introuvable dans le cache")
        ...
        # Filtrer les documents pour ce client avec vérification approfondie
        filtered_docs = []
        for doc in self.model.documents:
            doc_client_id = doc.get("client_id")
            # Vérification stricte de l'ID client
            if str(doc_client_id) == str(client_id):  # Conversion en string pour comparaison cohérente
                filtered_docs.append(doc)
                logger.debug(f"Document trouvé pour le client {client_id}: {doc.get('title', 'Sans titre')}")
        
        logger.info(f"Nombre de documents trouvés pour le client {client_name} ({client_id}): {len(filtered_docs)}")
        
        # Si aucun document trouvé mais que le client existe
        if not filtered_docs and client_exists:
            logger.warning(f"Aucun document trouvé pour le client {client_name} ({client_id})")
            self.no_documents_label.configure(
                text=f"Aucun document disponible pour le client {client_name}"
            )
            self.no_documents_label.pack(pady=20)
            return
    ...
    def _show_custom_folders(self):
        """Affiche les dossiers personnalisés - version corrigée"""
        # Nettoyer la vue
        self.documents_grid.pack_forget()
        self.no_documents_label.pack_forget()
    ...
    def create_document_card(self, document, row, col):
        ...
        if client_id:
            # Méthode optimisée pour récupérer le nom du client
            client_name = self._get_client_name_cached(client_id)
    ...
    def download_document(self, document_id):
        ...
        try:
            ...
            DialogUtils.show_message(
                self.parent,
                "Erreur",
                f"Erreur lors du téléchargement du document: {str(e)}",
                "error"
            )
