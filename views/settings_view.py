        ...
        try:
            ...
            for key, var in self._variables.items():
                try:
                    ...
                    logger.error(f"Erreur lors de la lecture de la variable {key}: {e}")
                    continue
            
            # Valider les paramètres
            is_valid, error_message = self._validate_settings(settings)
            if not is_valid:
                self.parent.after(100, lambda: DialogUtils.show_message(
                    self.parent,
            ...
            # Exécuter la sauvegarde après un court délai
            self.parent.after(100, do_save)
            
        except Exception as e:
            logger.error(f"Erreur lors de la préparation de la sauvegarde: {e}")
            self._save_in_progress = False
            self.parent.after(100, lambda: DialogUtils.show_message(
                self.parent,
                "Erreur",
                f"Erreur lors de la préparation de la sauvegarde:\n{str(e)}",
                "error"
            ))
