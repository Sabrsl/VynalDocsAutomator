...
class AdminModel:
    ...
    def add_user(self, user_data):
        """Ajoute un nouvel utilisateur avec gestion du cache"""
        success = super().add_user(user_data)
        if success:
    ...
    def update_user(self, user_data):
        ...
        success = super().update_user(user_data)
        if success:
            self._invalidate_cache('users')
        return success
    
    def delete_user(self, user_id):
        """Supprime un utilisateur avec gestion du cache"""
    ...
    def set_main_view(self, main_view):
        """
        Définit une référence à l'interface principale
        
        Args:
            main_view: Instance de AdminMain
        """
        self.main_view = main_view
        logger.debug("Référence à l'interface principale définie")

    def show_password_reset(self):
        """
        Affiche le gestionnaire de réinitialisation de mot de passe
        """
        if hasattr(self, 'main_view') and hasattr(self.main_view, 'show_password_reset_view'):
            self.main_view.show_password_reset_view()
            logger.info("Affichage du gestionnaire de réinitialisation de mot de passe")
        else:
            logger.error("Impossible d'afficher le gestionnaire de réinitialisation de mot de passe")
