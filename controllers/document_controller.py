...
class DocumentController:
    ...
    def delete_document(self, document_id):
        ...
        def delete_action():
            try:
                ...
                if "file_paths" in document:
                    files_to_delete.extend(document["file_paths"].values())
                elif "file_path" in document and document["file_path"]:
                ...
                for file_path in files_to_delete:
                    try:
                        ...
                        logger.warning(f"Erreur lors de la suppression du fichier {file_path}: {file_error}")
    ...
    def export_document(self, document_id, format_type=None):
        ...
        Args:
            ...
            format_type: Format de sortie (pdf ou docx)
            
        Returns:
            str: Chemin du fichier généré ou None en cas d'erreur
        """
        try:
            # Récupérer le document
            ...
            # Récupérer le modèle et le client associés
            template = self.model.get_template(document.get('template_id'))
            client = self.model.get_client(document.get('client_id'))
            ...
            doc_filename = self.generate_filename(
                template.get("type", "document"),
                client.get('name', 'client'),
                document.get('date', datetime.now().strftime("%Y-%m-%d"))
            )
            
            # Déterminer le chemin de sortie
            output_dir = self.model.paths['documents']
            output_path = os.path.join(output_dir, f"{doc_filename}.{format_type}")
            ...
            if success:
                ...
                self.model.update_document(document_id, document)
                
                logger.info(f"Document exporté avec succès: {output_path}")
                return output_path
            else:
                logger.error("Échec de la génération du document")
                return None
            
        except Exception as e:
            logger.error(f"Erreur lors de l'export du document: {e}")
            return None
