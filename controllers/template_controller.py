...
class TemplateController:
    ...
    def use_template(self, template_id):
        ...
        try:
            ...
            current = self.view.parent
            while current and not isinstance(current, ctk.CTk):
                current = current.master
            
            if not current:
                raise Exception("Vue principale non trouvée")
            
            # La fenêtre principale contient la vue principale comme premier enfant
            main_view = None
            for child in current.winfo_children():
                if hasattr(child, "views"):
                    main_view = child
                    break
